...
let bitFilter sorter input =
    let rec loop idx bits =
        let filter (bit: string) = bit.[idx] = (mostCommonValue bits idx sorter)
...
let lifeSupport input =
    o input * co2 input

[<EntryPoint>]
let main argv =
    let input = File.ReadAllLines "input" |> Seq.toList
